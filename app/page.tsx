...
const audiences = [
  ...
  ["02", "Creators e influenciadores", "Audiência que pode virar produto, receita e empresa."],
      ...
      <main id="conteudo">
        <section className="hero" id="inicio">
          <div className="shell hero-grid">
            <div className="hero-copy">
              ...
              <h1>Seu negócio tem valor.<br /><em>Nós fazemos ele funcionar.</em></h1>
              <p className="hero-lead">Encontramos o que está travando seu negócio — clientes, vendas ou operação — e construímos o próximo passo para ele crescer sem virar caos.</p>
            ...
            <div className="hero-visual" aria-label="Imagens da identidade OCOF">
          ...
          <div className="shell hero-signals" aria-label="Resumo da operação">
        ...
        <section className="section audience-section" id="publico">
          <div className="shell">
            ...
            <div className="section-heading-grid"><h2>Você já sabe fazer algo valioso. <em>Agora isso precisa funcionar como negócio.</em></h2><p className="section-copy">Especialistas, creators, consultores e empresas chegam com conhecimento, audiência, produto ou clientes. Nós transformamos esse ponto de partida em uma operação que vende, entrega e cresce.</p></div>
            <div className="audience-grid">{audiences.map(([index, title, text]) => <article className="audience-card" key={title}><span className="card-index">{index}</span><h3>{title}</h3><p>{text}</p><span className="card-arrow" aria-hidden="true">↗</span></article>)}</div>
          </div>
        </section>
        ...
        <section className="section dark-section blockers-section" id="gargalos">
          <div className="shell">
            ...
            <div className="blockers-grid">{blockers.map(([title, text, focus, index]) => <article className="blocker-card" key={title}><span className="card-index">{index}</span><h3>{title}</h3><p>{text}</p><div className="blocker-focus"><small>FOCO OCOF</small><strong>{focus}</strong></div><span className="card-arrow" aria-hidden="true">↗</span></article>)}</div>
          </div>
        </section>
        ...
        <section className="section method-section" id="metodo">
          <div className="shell">
            ...
            <ol className="method-grid">{method.map(([index, title, text]) => <li className="method-card" key={index}><span className="method-number">{index}</span><h3>{title}</h3><p>{text}</p></li>)}</ol>
          </div>
        </section>
        ...
        <section className="section closing-section" id="contato">
          <div className="shell closing-inner"><div className="section-kicker">Próximo estágio</div><h2>Seu negócio não precisa de mais uma ação.<br /><em>Precisa funcionar melhor.</em></h2><p>Encontramos o que trava, organizamos o que falta e construímos o próximo resultado.</p><a className="button button-light" href="mailto:contato@ocof.com.br">Descobrir o que trava <Arrow direction="right" /></a></div>
        </section>
      </main>
      <Footer />
    </>
